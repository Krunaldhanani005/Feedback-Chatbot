...
export function ProductCard({ product, onBookDemo, onContactUs }: ProductCardProps) {
  return (
    <div className="rounded-xl overflow-hidden transition-all duration-200"
      ...
      <div className="flex items-center gap-2 mb-1.5">
        <span className="text-xs text-brand-red font-semibold">{product.company}</span>
        {product.category && (
          <>
      ...
      <h4 className="font-bold text-sm text-text-primary mb-1">{product.name}</h4>
      <p className="text-xs text-text-secondary mb-2.5 line-clamp-2 leading-relaxed">{product.description}</p>
      ...
      {product.industries && product.industries.length > 0 && (
        <div className="flex flex-wrap gap-1 mb-3">
          {product.industries.slice(0, 4).map((ind, i) => (
